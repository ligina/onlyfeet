...
Evaluate a trained M14 task-specialist model on a validation/test npz.
Supports normal evaluation and inference-time modality ablation by zeroing inputs.
"""
...
NPZ_KEYS = {"imu": "imu_win", "tof": "tof_win", "mag": "mag_win", "audio": "audio_win", "image": "img_win"}
...
def normalize_using_self(x):
    # Use only when original training statistics were not persisted. This is suitable for quick robustness comparisons
    # within the same eval npz, but final numbers should come from training script's saved eval metrics.
    axes = tuple(range(x.ndim - 1))
    return np.nan_to_num((x - x.mean(axis=axes, keepdims=True)) / (x.std(axis=axes, keepdims=True) + 1e-6)).astype(np.float32)


def infer_modality_from_input_name(name):
    n = name.lower()
    if "imu" in n:
        return "imu"
    if "tof" in n:
...
def infer_modality_from_shape(expected):
    """Infer modality from input shape after H5 reload, where input names may be generic."""
    e = tuple(expected)
    if e == (50, 8) or e == (50, 6):
        return "imu"
    if e == (50, 2):
...
def is_compatible(x, expected):
    if tuple(x.shape[1:]) == tuple(expected):
        return True
    if len(x.shape[1:]) + 1 == len(expected) and tuple(x.shape[1:]) == tuple(expected[:-1]) and expected[-1] == 1:
        return True
    return False


def prepare_array(npz, modality, expected, no_imu_magnitude=False):
    if modality not in NPZ_KEYS:
    ...
    if len(arr.shape[1:]) + 1 == len(expected) and tuple(arr.shape[1:]) == tuple(expected[:-1]) and expected[-1] == 1:
        arr = arr[..., np.newaxis]
    if not is_compatible(arr, expected):
        raise ValueError(f"Array shape for {modality} incompatible. got={arr.shape[1:]}, expected={expected}")
    return normalize_using_self(arr)
...
def evaluate_condition(model, x_inputs, y_true, labels, out_dir, condition, batch):
    ...
    (out_dir / "classification_report.txt").write_text(classification_report(y_true, pred, target_names=labels, digits=4, zero_division=0), encoding="utf-8")
    report_dict = classification_report(y_true, pred, target_names=labels, digits=4, zero_division=0, output_dict=True)
    (out_dir / "classification_report.json").write_text(json.dumps(report_dict, indent=2), encoding="utf-8")
    p, r, f1, support = precision_recall_fscore_support(y_true, pred, labels=list(range(len(labels))), zero_division=0)
    ...
    np.savez_compressed(out_dir / "predictions.npz", y_true=y_true, y_pred=pred, pred_prob=prob)
    return metrics
...
def main():
    ...
    npz = np.load(args.eval_npz, allow_pickle=True)
    card = load_json(args.card)
    y_true, valid, labels = get_labels(npz, card, args.task)
    ...
    for inp in model.inputs:
        ...
        # Prefer shape-based inference after H5 reload.
        # H5 may restore generic or misleading input names, while shapes are unambiguous here.
        mod = infer_modality_from_shape(expected)
        if mod is None:
            mod = infer_modality_from_input_name(inp.name)
        if mod is None:
            raise ValueError(f"Cannot infer modality from model input: {inp.name}, shape={expected}")
        arr = prepare_array(npz, mod, expected, no_imu_magnitude=args.no_imu_magnitude)[valid]
        base_arrays.append(arr)
        input_info.append({"input_name": inp.name, "modality": mod, "expected_shape": list(expected), "array_shape": list(arr.shape)})
